...
class Propertystaging extends Controller
{
    ...
    public function addProperty()
    {
        ...
        if ($this->request->getMethod() == "post") {
            ...
            $fileUploadResponse = $this->uploadFiles($propertyData['p_code']);
    ...
    public function uploadFiles($p_code)
    {
        ...
        foreach ($uploadedFiles['images'] as $img) {
            // Handle each uploaded file (e.g., move it to a specific directory)
           
            
                $newName = $img->getRandomName();
                $img->move(FCPATH . 'public/contentuploads', $newName);
                $path = base_url('public/contentuploads/' . $newName);
                $imageData[] = [
                    'image_link' => $path,
                    'p_code' => $p_code,
                ];
            // You may want to store the file details in a database or perform other operations

        }
        $this->pModel->insertImages($imageData);

        return $this->response->setStatusCode(200)->setJSON(['message' => 'Files uploaded successfully']);
    }
}